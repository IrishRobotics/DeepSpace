package frc.team2606.lib.trajectory.timing;

import frc.team2606.lib.geometry.ICurvature;
import frc.team2606.lib.geometry.IPose2d;
import frc.team2606.lib.physics.DifferentialDrive;
import frc.team2606.lib.util.Units;
...
public class DifferentialDriveDynamicsConstraint<S extends IPose2d<S> & ICurvature<S>> implements TimingConstraint<S> {
    ...
    public double getMaxVelocity(S state) {
        return Units.meters_to_inches(
                drive_.getMaxAbsVelocity(Units.meters_to_inches(state.getCurvature()), abs_voltage_limit_));
    ...
    public MinMaxAcceleration getMinMaxAcceleration(S state, double velocity) {
        // TODO figure out a units convention for generic states. Traditionally we use
        // inches...
        // NOTE: units cancel on angular velocity.
        DifferentialDrive.MinMax min_max = drive_.getMinMaxAcceleration(
                new DifferentialDrive.ChassisState(Units.inches_to_meters(velocity), state.getCurvature() * velocity),
                Units.meters_to_inches(state.getCurvature()), // Curvature is in inverse inches, so meters_to_inches is
                                                              // correct.
                /*
                 * Units.meters_to_inches(Units.meters_to_inches(state.getDCurvatureDs())), //
                 * DCurvature is in inverse inches^2.
